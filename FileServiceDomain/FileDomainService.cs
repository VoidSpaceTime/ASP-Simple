...
namespace FileServiceDomain
{
    public class FileDomainService
    {
        ...
        public async Task<UploadedItem> UploadAsync(Stream stream, string fileName, CancellationToken cancellationToken)
        {
            ...
            string extension = Path.GetExtension(fileName); // 返回 扩展名
            ...
            // 我不同意 , 我非要用GUID作为文件名
            string savePath = $"{today.Year}/{today.Month}/{today.Day}/{id + extension}";
            stream.Position = 0;
            //backupStorage实现很稳定、速度很快，一般都使用本地存储（文件共享或者NAS）
            Uri backupUrl = await backupStorage.SaveAsync(savePath, stream, cancellationToken);//保存到本地备份
            stream.Position = 0;
            Uri remoteUrl = await remoteStorage.SaveAsync(savePath, stream, cancellationToken);//保存到生产的存储系统
            stream.Position = 0;
